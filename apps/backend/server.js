...
app.post('/update-latest-budget-number', async (req, res) => {
    try {
        ...
        if (latest_budget_number === undefined) {
            return res.status(400).send('Nenhum dado de orçamento recebido.');
        }
        ...
        while (!isUpdated && attempts < maxAttempts) {
            await new Promise(r => setTimeout(r, 5000)); // Espera 5 segundos antes de verificar novamente
            const githubPagesResponse = await axios.get(`https://nicholas1front.github.io/emeg_system/${LATEST_BUDGET_FILE_PATH}`);
            const currentContent = await githubPagesResponse.data;

            if (JSON.stringify(currentContent) === JSON.stringify({ latest_budget_number })) {
                isUpdated = true; // O arquivo no GitHub Pages foi atualizado
            }
            attempts++;
        }
